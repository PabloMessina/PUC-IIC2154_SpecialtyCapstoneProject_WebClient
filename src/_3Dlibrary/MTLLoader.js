...
const MTLLoader = {
   ...
   */
  loadMaterials: (mtlFile, texturePaths) => (
    new Promise((resolve, reject) => {
      ...
      fr.onload = () => {
        ...
        // if we tried to load at least one texture asynchronously, and therefore
        ...
        if (promiseList.length > 0) {
          ...
          .catch(err => { // error
            console.log("----------------");
            console.log("MTLLoader: rejecting promise, some of the promises went wrong");
            console.log("err = ", err);
            reject(err);
          });
        ...
        function trySetTextureAsynchronously(textureName, mapName) {
          if (promiseMap[textureName] === undefined) {
            ...
            if (path === undefined) {
              ...
              // and return false
              console.log("trySetTextureAsynchronously(): rejecting promise, path = undefined");
              reject({ reason: 'path to image ' + textureName + ' not found' });
              return false;
            }
            ...
            const p = new Promise((res, rej) => {
              loader.load(
                ...
                null, (err) => {
                  console.log("----------");
                  console.log("loader.load(): rejecting outer promise");
                  console.log("\tpath=",path," textureName=",textureName);
                  console.log("err = ",err);
                  rej(err);
                }
              );
      ...
      // set error function
      fr.onerror = (err) => {
        console.log("MTLLoader: rejecting promise because fr.onerror");
        console.log("err = ", err);
        reject(err);
      };
